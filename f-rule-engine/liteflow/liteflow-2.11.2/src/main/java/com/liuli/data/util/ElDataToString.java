...
import com.liuli.data.entity.LiteFlowParseException;
...
public class ElDataToString {
     ...
     * @author zhanghui
     * @date 2023/10/17 15:46:40
     * @see ElReceiveData
     * @see ELWrapper
     * @since 1.0.0
     */
    public static ELWrapper convertObjectToELWrapper(ElReceiveData receiveData) {
        List<ELWrapper> childrenWrapper = new ArrayList<>(); // 用于存储子级ELWrapper
        List<ElReceiveData> childrenList = receiveData.getChildrenList(); // 获取子级数据列表
        if (CollectionUtil.isNotEmpty(childrenList)) { // 如果子级数据不为空，进行递归转换
            for (ElReceiveData data : childrenList) {
                ELWrapper elWrapper = convertObjectToELWrapper(data); // 递归构建子级的 ELWrapper
                childrenWrapper.add(elWrapper);
            }
        }
        ELWrapper elWrapper = getWrapper(receiveData, childrenWrapper); // 获取包装好的ELWrapper对象
        return elWrapper;
    }
    ...
    private static ELWrapper getWrapper(ElReceiveData receiveData, List<ELWrapper> childrenWrapper) {
        ...
        String id = receiveData.getId(); // 从ElReceiveData获取id
        String tag = receiveData.getTag(); // 从ElReceiveData获取tag
        String typeName = receiveData.getType().getName(); // 从ElReceiveData获取typeName
        List<String> nodeIdList = receiveData.getNodeIdList(); // 从ElReceiveData获取nodeIdList
        ...
        switch (typeName) {
            ...
            case "CatchELWrapper":
                ...
                break;
            case "IfELWrapper":
                if(StrUtil.isNotBlank(receiveData.getTrueNodeId())){
                    if (StrUtil.isNotBlank(receiveData.getFalseNodeId())){
                        elWrapper = ELBus.ifOpt(nodeIdList.get(0), receiveData.getTrueNodeId(),receiveData.getFalseNodeId());
                    }else {
                        elWrapper = ELBus.ifOpt(nodeIdList.get(0), receiveData.getTrueNodeId());
                    }
                }else {
